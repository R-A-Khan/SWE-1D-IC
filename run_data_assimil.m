...
x0_min      = 0.1;        % Location of first observation point 
...
bath       = false;        % Include non-flat bottom bathymetry
...
%         dmu  = abs((xmax-1e-4) - x0_min)/n_obs(k);
        [eta_optimum(:,k,j_nl), eta_exct0(:,k,j_nl),bathymetry(:,k,j_nl), X, err(:,k,j_nl), err_std(:,k,j_nl), grad(:,k,j_nl), grad_std(:,k,j_nl), cost(:,k,j_nl), cost_std] = ...
            data_assimil(nl,N,j_obs,rand_x0,x0_min,dmu,ntrial,tmax,xmax,iter_max,line_min,smooth_grad,filt, bath);
    end
    disp(' ');
end
% Save results
save('Assimilation_for_SOA', 'ntrial', 'n_obs', 'rand_x0','x0_min', 'dmu', 'line_min','smooth_grad', 'iter_max', ...
    'eta_optimum', 'bathymetry', 'eta_exct0', 'X', 'err', 'err_std', 'grad', 'grad_std', 'cost', 'cost_std');
%% Plotting
clear all;
%

load('Assimilation_for_SOA');
clf;
iter = 0:iter_max;
