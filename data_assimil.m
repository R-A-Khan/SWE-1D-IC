function [eta_optimum, eta_exct0, beta, X, err, err_std, grad, grad_std, cost, cost_std, n_iter] = ...
    data_assimil(nl,N,n_obs,rand_x0,x0_min,dmu,ntrial,tmax,xmax,iter_max,line_min,smooth_grad,filt,bath, bath_func)
% Usage: 
...
%     x0_vals
    ...
    amp = 0.1; eta_exct0 = amp.*exp(-X.^2/0.1^2);
    ...
    while norm(grad_J) >= 1e-20 && iter<iter_max
        ...
        cost(iter+1,jtrial) = trapz(T2,0.5*cost_x);
        ...
        sprintf('%0.3e',norm(err(iter,jtrial)))]);
%         
