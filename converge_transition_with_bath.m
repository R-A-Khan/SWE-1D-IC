...
bath_func = @(x) 0.1.*exp(-(x-1.5).^2/0.1^2); % function for bottom topography
...
for j_nl=1:2
    ...
    for j_obs = n_obs
        ...
        for l = 1:numel(dmu)
            ...
            data_assimil(nl,N,j_obs,rand_x0,x0_min,dmu(l),ntrial,tmax,xmax,iter_max,line_min,smooth_grad,filt, bath, bath_func);
        end
    end
    disp(' ');
end
% Save results
save('IC_Assimilation_with_bathy_0_1_amp', 'ntrial', 'n_obs', 'rand_x0','x0_min', 'dmu', 'line_min','smooth_grad', 'iter_max', ...
