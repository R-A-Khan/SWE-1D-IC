...
%     beta = b_amp.*(tanh(2*(X+2*ones(N,1))) - tanh(2*(X-2*ones(N,1))) );
    ...
    % STEP 2.1: Distort exact IC to get "guess" IC eta0(:,1) used in algorithm with analytical grad_J
    eta0 = 0.9*eta_exct0;
    ...
    % Solve tangent model for perturbed system eta + ep*eta_hat
    u = zeros(N,1);
    eta_hat0 = amp*sin(X);
    H_hat = [eta_hat0 ; u];
    [~, ~, eta_hat_x0_pts, test_nl, T_hat, Y_hat] = FW_solve_tangent(h, sz, H_hat, 0, tmax, u_all, eta_all, x0_inds, fw_tangent);
    eta_hat = Y_hat(1:N,:);
    u_hat = Y_hat(N+1:2*N,:);
    
    % Solve SOA model for eta_bat(x,0)
    H_bar = zeros(2*N,1);
    [eta_bar_t0_x_nl, T_bar, Y_bar] = BW_solve_SOA(h, sz, H_bar, 0, tmax, x0_inds, bw_SOA, eta_hat_x0_pts, u_all, eta_all, u_adj, eta_adj, u_hat, eta_hat, T_hat, T2, nl);
    
    % Define Hessian
    Hessian_J =  - eta_bar_t0_x_nl;
...
%     psi_equiv_2 = (0.5*ep(i)^2)* trapz(X,(trapz(X,eta0_prime_g.*Hessian_J)) .* eta0_prime_g)
    psi_equiv_2 =  trapz(X, (- eta_bar_t0_x_nl.* eta0_prime_g ));
...
set(gca,'FontSize',22)
legend('Nonlinear SWE', 'location','northeastoutside');
title('Kappa test for Hessian $\nabla^2 \mathcal{J}_{\eta_0}$- Gateaux Definition of $\mathcal{J}\prime(\eta_0;\eta\prime)$', 'interpreter', 'latex')
xlabel('log(\epsilon)')
ylabel('|\psi(\epsilon)|');
...
legend('Nonlinear SWE', 'location','northeastoutside');
xlabel('log(\epsilon)');
ylabel('log|\psi(\epsilon)-1|');

figure(5);
semilogx((ep(1:length(psi_2))),abs(psi_2),'k',  'linewidth',1.5); grid on;
set(gca,'FontSize',22)
legend('Nonlinear SWE', 'location','northeastoutside');
title('Kappa test for Hessian $\nabla^2 \mathcal{J}_{\eta_0}$- Riesz Definition of $\mathcal{J}\prime(\eta_0;\eta\prime)$', 'interpreter', 'latex')
xlabel('log(\epsilon)')
